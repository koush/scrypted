...
if (!cluster.isMaster) {
    ...
    let debugWorker: number = undefined;
    ...
    net.createServer(socket => {
        ...
        const target = net.connect(process.debugPort + debugWorker);
    ...
    app.use(bodyParser.raw({ type: 'application/zip', limit: 20000000 }))
    ...
    async function start() {
        ...
        app.post('/web/component/script/debug', async (req, res) => {
            ...
            const pluginHost = await scrypted.installPlugin(plugin, true);
            debugWorker = pluginHost.worker.id;
